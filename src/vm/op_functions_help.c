...
int 	get_arg(int type_code, int pos, int arg_size)
{
	int arg;
	int i;
	int     p;//i dont know if it is wright
	...
	else if (type_code == IND_CODE)
	{
		while (arg_size > 0)
			arg += (g_vm->map[(pos++) % MEM_SIZE] << (8 * --arg_size));
		arg = (p + (arg % IDX_MOD)) % MEM_SIZE;//the thaim
	}
	return (arg);
}
...
void		get_all_arg(int *arg, int num_arg, t_car *c)
{
	...
	while (++a < num_arg)
	{
		...
		{
			arg[a] = get_arg(REG_CODE, (c->pos + step) % MEM_SIZE, 0);
			step += 1;
		}
		if (g_vm->arg_type[a] == DIR_CODE)
		{
			arg[a] = (g_op[c->op_id].dir_size == 2) ?
			(short)get_arg(DIR_CODE, (c->pos + step) % MEM_SIZE, g_op[c->op_id].dir_size) :
			get_arg(DIR_CODE, (c->pos + step) % MEM_SIZE, g_op[c->op_id].dir_size);
			step += g_op[c->op_id].dir_size;
		}
		if (g_vm->arg_type[a] == IND_CODE)
		{
			arg[a] = get_arg(IND_CODE, (c->pos + step) % MEM_SIZE, 2);
			step += 2;
		}
	}
}
